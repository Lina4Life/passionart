...
function Admin() {
  ...
  const [addArticleData, setAddArticleData] = useState({ title: '', content: '', category: '', tags: '' });
  ...
  // ensure articles state is set to what backend returns
  setArticles(articlesData || []);
  ...
  const handleAddArticleClick = () => {
    setAddArticleError('');
    setAddArticleData({ title: '', content: '', category: '', tags: '' });
  ...
  const handleAddArticleChange = (e) => {
    setAddArticleData({ ...addArticleData, [e.target.name]: e.target.value });
  };
  ...
  const handleAddArticleSubmit = async (e) => {
    ...
    try {
      // call API
      const payload = { ...addArticleData };
      // author_id will be null or current user id if available
      if (user && user.id) payload.author_id = user.id;
      const resp = await createArticle(payload);
      ...
      setAddArticleError(err?.response?.data?.error || 'Failed to create article');
    } finally {
  ...
  const renderArticles = () => (
    <div className="admin-section">
      <div className="section-header">
        ...
        <button className="admin-btn primary" onClick={handleAddArticleClick}>Add Article</button>
      </div>
      <div className="table-container">
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleAddArticleSubmit} className="modal-form">
              ...
              <div className="form-group">
                ...
                <input name="title" value={addArticleData.title} onChange={handleAddArticleChange} className="form-input" required disabled={addArticleLoading} />
              </div>
              <div className="form-group">
                <label className="form-label">Category</label>
                <input name="category" value={addArticleData.category} onChange={handleAddArticleChange} className="form-input" disabled={addArticleLoading} />
              </div>
              <div className="form-group">
                <label className="form-label">Tags (comma separated)</label>
                <input name="tags" value={addArticleData.tags} onChange={handleAddArticleChange} className="form-input" disabled={addArticleLoading} />
              </div>
              <div className="form-group">
                <label className="form-label">Content *</label>
                <textarea name="content" value={addArticleData.content} onChange={handleAddArticleChange} className="form-input" rows="8" required disabled={addArticleLoading} />
              </div>
              <div className="modal-actions">
                <button type="button" className="admin-btn" onClick={handleAddArticleCancel} disabled={addArticleLoading}>Cancel</button>
                <button type="submit" className="admin-btn primary" disabled={addArticleLoading}>{addArticleLoading ? 'Creating...' : 'Create Article'}</button>
              </div>
            </form>
          </div>
        </div>
