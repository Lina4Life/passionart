...
const getAllArticles = async (req, res) => {
  ...
  const createArticle = async (req, res) => {
    try {
      const { title, content, category, tags, author_id, status, published_at, featured_image } = req.body;
      ...
      const insertQuery = `
        INSERT INTO articles (title, content, category, tags, author_id, status, published_at, featured_image, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      ...
      db.run(insertQuery, [
        ...
        res.status(201).json({ success: true, article: { id: newId.toString(), title, content, category, tags, author_id, status, published_at: published_at || createdAt, featured_image, created_at: createdAt } });
      });
    } catch (error) {
